...
Postgres, arranca el host (Eventuous crea el schema), corre el workflow y
...
// 2. Arrancar el host → el SchemaInitializer de Eventuous crea curso_eventstore
//    y arrancan los hosted services (la suscripción del read model).
await host.StartAsync(ct);
await WaitForEventStoreObjectsAsync(connectionString, ct);

// 3. Inicializar las tablas del read model (idempotente).
...
arranca el host** (es un hosted service). Por eso la demo:
